...
class MarketAnalyzer:
    ...
    def analyze_market_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            prices = [price.last_traded_price for price in share_prices if price.last_traded_price > 0]
            changes = [price.change_percentage for price in share_prices 
            ...
            analysis = {
                ...
                "market_sentiment": {
                    ...
                    "avg_gain": statistics.mean(positive_changes) if positive_changes else 0,
                    "avg_loss": statistics.mean(negative_changes) if negative_changes else 0,
                    "market_breadth": len(positive_changes) / len(changes) if changes else 0,
                }
            }
    ...
    def find_opportunities(self) -> Dict[str, List[SharePrice]]:
        ...
        try:
            ...
            for price in share_prices:
                ...
                if (price.change_percentage > 5.0 and 
                    price.volume and price.volume > 10000):
                ...
                elif (price.last_traded_price < 50 and 
            ...
            for key in opportunities:
                if key == "oversold_stocks":
                    ...
                    opportunities[key].sort(key=lambda x: x.change_percentage, reverse=True)
    ...
    def sector_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Sort by performance
            best_sectors = sorted(sector_performance, 
                                key=lambda x: x["change_percentage"], reverse=True)
            worst_sectors = sorted(sector_performance, 
...
def main():
    ...
    if market_analysis:
        ...
        print(f"ASPI: {market_analysis['aspi']['value']:.2f} "
              f"({market_analysis['aspi']['change']:+.2f})")
        print(f"S&P SL20: {market_analysis['snp_sl20']['value']:.2f} "
    ...
    if opportunities:
        ...
        if opportunities['value_picks']:
            ...
            for stock in opportunities['value_picks'][:5]:
                print(f"  {stock.symbol}: LKR {stock.last_traded_price:.2f} "
